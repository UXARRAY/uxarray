...
class TestAPI(TestCase):
    ...
    def test_open_geoflow_dataset(self):
        ...
        data_paths = [self.geoflow_data_v1, self.geoflow_data_v2, self.geoflow_data_v3]
    ...
    def test_open_dataset(self):
        ...
        uxds_var2_ne30 = ux.open_dataset(self.gridfile_ne30, self.dsfile_var2_ne30)

        nt.assert_equal(uxds_var2_ne30.uxgrid.node_lon.size, constants.NNODES_outCSne30)
        nt.assert_equal(
            len(uxds_var2_ne30.uxgrid._ds.data_vars), constants.DATAVARS_outCSne30
        )
        nt.assert_equal(uxds_var2_ne30.source_datasets, str(self.dsfile_var2_ne30))

    def test_open_mf_dataset(self):
        """Loads multiple datasets with their grid topology file using
        uxarray's open_dataset call."""

        uxds_mf_ne30 = ux.open_mfdataset(self.gridfile_ne30, self.dsfiles_mf_ne30)

        nt.assert_equal(uxds_mf_ne30.uxgrid.node_lon.size, constants.NNODES_outCSne30)
        nt.assert_equal(
            len(uxds_mf_ne30.uxgrid._ds.data_vars), constants.DATAVARS_outCSne30
        )
    ...
    def test_open_grid(self):
        ...
        nt.assert_almost_equal(
            uxgrid.calculate_total_face_area(), constants.MESH30_AREA, decimal=3
        )
