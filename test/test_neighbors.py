...
class TestBallTree(TestCase):
    ...
    def test_query_radius(self):
        ...
        for grid_file in self.center_grid_files:
            ...
            ind = uxgrid.get_ball_tree(
                coordinates="face centers",
                coordinate_system="spherical",
                distance_metric="haversine",
                reconstruct=True,
            ).query_radius([3.0, 3.0], r=15, return_distance=False)
            ...
            d, ind = uxgrid.get_ball_tree(
                ...
                distance_metric="minkowski",
                reconstruct=True,
            ).query_radius([0.0, 0.0, 1.0], r=15, return_distance=True)
    ...
    def test_query(self):
        ...
        ).query([0.0, 0.0], return_distance=True, k=3)
        ...
        ind = uxgrid.get_ball_tree(
            coordinates="face centers", coordinate_system="spherical"
        ).query([0.0, 0.0], return_distance=False, k=3)
    ...
    def test_multi_point_query(self):
        ...
        for grid_file in self.center_grid_files:
            ...
            for i, cur_c in enumerate(c):
                single_ind = ind = uxgrid.get_ball_tree(
                    coordinates="nodes"
                ).query_radius(cur_c, 45)
...
class TestKDTree(TestCase):
    ...
    def test_construction_using_spherical_coords(self):
        ...
        for grid_file in self.corner_grid_files:
            uxgrid = ux.open_grid(grid_file)
            d, ind = uxgrid.get_kd_tree(
                coordinates="nodes", coordinate_system="spherical"
            ).query([3.0, 3.0], k=5)
    ...
    def test_query_radius(self):
        ...
        d, ind = uxgrid.get_kd_tree(
            coordinates="face centers", coordinate_system="spherical", reconstruct=True
        ).query_radius([3.0, 3.0], r=5, return_distance=True)

        # assert the distance and indexes have been populated
        self.assertTrue(len(d), len(ind))
        # Test returning just the indexes
        ind = uxgrid.get_kd_tree(
            coordinates="face centers", coordinate_system="spherical", reconstruct=True
        ).query_radius([3.0, 3.0], r=5, return_distance=False)
    ...
    def test_query(self):
        ...
        d, ind = uxgrid.get_kd_tree(
            coordinates="face centers", coordinate_system="spherical"
        ).query([0.0, 0.0], return_distance=True)
        ...
        ind = uxgrid.get_kd_tree(
            coordinates="face centers", coordinate_system="cartesian", reconstruct=True
        ).query([0.0, 0.0, 1.0], return_distance=False)
    ...
    def test_multi_point_query(self):
        ...
        for grid_file in self.center_grid_files:
            ...
            for i, cur_c in enumerate(c):
                single_ind = uxgrid.get_kd_tree(coordinates="nodes").query_radius(
                    cur_c, 45
                )
