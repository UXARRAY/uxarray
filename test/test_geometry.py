...
class TestPredicate(TestCase):
    def test_pole_point_inside_polygon_from_vertice_north(self):
        ...
        result = ux.grid.geometry._pole_point_inside_polygon("North", face_edge_cart)
        self.assertTrue(result, "North pole should be inside the polygon")

        # Check if the South pole is inside the polygon
        result = ux.grid.geometry._pole_point_inside_polygon("South", face_edge_cart)
    ...
    def test_pole_point_inside_polygon_from_vertice_pole(self):
        ...
        vertices = [[0, 0, 1], [-0.5, 0.5, 0.5], [-0.5, -0.5, 0.5], [0.5, -0.5, 0.5]]
...
class TestLatlonBoundUtils(TestCase):
    def _max_latitude_rad_iterative(self, gca_cart):
        ...
        while (
            np.abs(b_lonlat[1] - c_lonlat[1]) >= ERROR_TOLERANCE
            or np.abs(b_lonlat[0] - c_lonlat[0]) >= ERROR_TOLERANCE
        ):
            max_lat = -np.pi
            v_b, v_c = max_section
            ...
            for i in range(10):
                ...
                angle_rad_next = (
                    angle_rad_prev + avg_angle_rad if i < 9 else angle_v1_v2_rad
                )
                ...
                if (
                    np.abs(w2_lonlat[1] - w1_lonlat[1]) <= ERROR_TOLERANCE
                    or w1_lonlat[1] == max_lat == w2_lonlat[1]
                ):
    ...
    def _min_latitude_rad_iterative(self, gca_cart):
        ...
        while (
            ...
            for i in range(10):
                ...
                if (
                    np.abs(w2_lonlat[1] - w1_lonlat[1]) <= ERROR_TOLERANCE
                    or w1_lonlat[1] == min_lat == w2_lonlat[1]
                ):
    ...
    def test_extreme_gca_latitude_max(self):
        ...
        self.assertAlmostEqual(
            max_latitude, expected_max_latitude, delta=ERROR_TOLERANCE
        )
    ...
    def test_extreme_gca_latitude_min(self):
        ...
        self.assertAlmostEqual(
            min_latitude, expected_min_latitude, delta=ERROR_TOLERANCE
        )
    ...
    def test_populate_bounds_normal(self):
        ...
        lat_max = max(
            ...
            extreme_gca_latitude(
                np.array([vertices_cart[0], vertices_cart[3]]), extreme_type="max"
            ),
        )
        lat_min = min(
            np.deg2rad(10.0),
            extreme_gca_latitude(
                np.array([vertices_cart[1], vertices_cart[2]]), extreme_type="min"
            ),
        )
        ...
        bounds = _populate_face_latlon_bound(
            face_edges_connectivity_cartesian, face_edges_connectivity_lonlat
        )
...
class TestLatlonBoundsLatLonFace(TestCase):
    def test_populate_bounds_normal(self):
        ...
        bounds = _populate_face_latlon_bound(
            ...
            face_edges_connectivity_lonlat,
            is_latlonface=True,
        )
        nt.assert_allclose(bounds, expected_bounds, atol=ERROR_TOLERANCE)
...
class TestLatlonBoundsMix(TestCase):
    ...
    def test_populate_bounds_LatlonFace_mix(self):
        ...
        expected_bounds = [
            [
                ...
                [np.deg2rad(10.0), np.deg2rad(50.0)],
            ],
            [
                [np.deg2rad(10.0), np.deg2rad(60.0)],
                [np.deg2rad(350.0), np.deg2rad(50.0)],
            ],
            [[np.deg2rad(60.0), np.pi / 2], [np.deg2rad(210.0), np.deg2rad(30.0)]],
            [[np.deg2rad(60.0), np.pi / 2], [0.0, 2 * np.pi]],
        ]
    ...
    def test_populate_bounds_GCAList_mix(self):
        ...
        grid = ux.Grid.from_face_vertices(faces, latlon=True)
        bounds_xarray = _populate_bounds(
            grid, is_face_GCA_list=[[True, False, True, False]] * 4, return_array=True
        )
        face_bounds = bounds_xarray.values
        for i in range(len(faces)):
            nt.assert_allclose(face_bounds[i], expected_bounds[i], atol=ERROR_TOLERANCE)
...
class TestGeoDataFrame(TestCase):
    ...
    def test_cache_and_override(self):
        ...
        gdf_e = uxgrid.to_geodataframe(
            exclude_antimeridian=True, override=True, cache=False
        )
