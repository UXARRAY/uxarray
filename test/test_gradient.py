...
class TestGrad(TestCase):
    ...
    quad_hex_grid_path = (
        current_path / "meshfiles" / "ugrid" / "quad-hexagon" / "grid.nc"
    )
    quad_hex_data_path = (
        current_path / "meshfiles" / "ugrid" / "quad-hexagon" / "data.nc"
    )
    ...
    def test_uniform_data(self):
        ...
        for grid_path in [
            self.mpas_atmo_path,
            self.mpas_ocean_path,
            self.CSne30_grid_path,
            self.quad_hex_grid_path,
        ]:
            uxgrid = ux.open_grid(grid_path)

            uxda_zeros = ux.UxDataArray(
                data=np.zeros(uxgrid.n_face),
                uxgrid=uxgrid,
                name="zeros",
                dims=["n_face"],
            )
            ...
            uxda_ones = ux.UxDataArray(
                data=np.ones(uxgrid.n_face),
                uxgrid=uxgrid,
                name="zeros",
                dims=["n_face"],
            )
    ...
    def test_grad_multi_dim(self):
        ...
        uxda = ux.UxDataArray(
            uxgrid=uxgrid, data=sample_data, dims=["time", "lev", "n_face"]
        )
