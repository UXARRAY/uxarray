...
class PointsAlongGCA:
    def __init__(self):
        ...
        self.points = []
        # Maps point tuple -> index in self.points
        self.point_to_index = {}
        # Set of edges (i, j) where i and j are indices in self.points
        self.edges = set()
        # List of points inserted as singles (waiting to be connected)
        self.singles = []
    ...
    def insert_pair(self, p1, p2):
        """Insert a pair of points and connect them with an edge."""
        # Convert inputs to NumPy arrays if they aren't already
        p1 = np.asarray(p1)
        p2 = np.asarray(p2)

        p1_key = self._point_to_key(p1)
        if p1_key not in self.point_to_index:
            idx1 = len(self.points)
            self.points.append(p1)
            self.point_to_index[p1_key] = idx1
        else:
            idx1 = self.point_to_index[p1_key]

        p2_key = self._point_to_key(p2)
        if p2_key not in self.point_to_index:
            idx2 = len(self.points)
            self.points.append(p2)
            self.point_to_index[p2_key] = idx2
        else:
            idx2 = self.point_to_index[p2_key]

        # Record the edge
        self.edges.add((idx1, idx2))

    def insert_single(self, p):
        """Insert a single point to be connected later during finalization."""
        p = np.asarray(p)
        p_key = self._point_to_key(p)

        if p_key not in self.point_to_index:
            idx = len(self.points)
            self.points.append(p)
            self.point_to_index[p_key] = idx

        # Keep track of it for future connection
        self.singles.append(p)

    def finalize(self):
        """Connect all single points to their nearest neighbors."""
        points_array = np.array(self.points)

        for single_point in self.singles:
            single_idx = self.point_to_index[self._point_to_key(single_point)]

            distances = np.sqrt(np.sum((points_array - single_point) ** 2, axis=1))
            distances[single_idx] = float("inf")

            closest_idx = np.argmin(distances)

            self.edges.add((single_idx, closest_idx))

        self.singles.clear()

    def get_all_edges(self):
        """Return list of point pairs representing edges."""
        return [(self.points[i], self.points[j]) for i, j in self.edges]

    def get_points_as_array(self):
        """Return all points as a single NumPy array."""
