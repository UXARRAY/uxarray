...
class Grid:
    ...
    def to_planar(self):
        """Constructs a planar representation of the grid, and returns a dataset containing nodes, faces, and edges."""

        node_x_planar, node_y_planar = _point_to_plane(
            self.node_x.values, self.node_y.values, self.node_z.values
        )

        face_x_planar, face_y_planar = _point_to_plane(
            self.face_x.values, self.face_y.values, self.face_z.values
        )

        edge_x_planar, edge_y_planar = _point_to_plane(
            self.edge_x.values, self.edge_y.values, self.edge_z.values
        )

        planar_grid = xr.Dataset()

        planar_grid["node_x_planar"] = xr.DataArray(
            node_x_planar, dims=[ugrid.NODE_DIM], attrs=None
        )
        planar_grid["node_y_planar"] = xr.DataArray(
            node_y_planar, dims=[ugrid.NODE_DIM], attrs=None
        )

        planar_grid["face_x_planar"] = xr.DataArray(
            face_x_planar, dims=[ugrid.FACE_DIM], attrs=None
        )
        planar_grid["face_y_planar"] = xr.DataArray(
            face_y_planar, dims=[ugrid.FACE_DIM], attrs=None
        )

        planar_grid["edge_x_planar"] = xr.DataArray(
            edge_x_planar, dims=[ugrid.EDGE_DIM], attrs=None
        )
        planar_grid["edge_y_planar"] = xr.DataArray(
            edge_y_planar, dims=[ugrid.EDGE_DIM], attrs=None
        )
