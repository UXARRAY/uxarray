...
def close_face_nodes(face_nodes, n_face, nMax_face_nodes):
    """Closes (``face_nodes``) by inserting the first node index after the last
    non-fill-value node.
    ...
    n_face : constant
        Number of faces
    nMax_face_nodes : constant
        Max number of nodes that compose a face

    Returns
    ----------
    closed : ndarray
        Closed (padded) Mface_nodes
    ...
    # padding to shape [n_face, nMax_face_nodes + 1]
    closed = np.ones(
        (n_face, nMax_face_nodes + 1), dtype=INT_DTYPE) * INT_FILL_VALUE
    ...
    first_fv_idx_1d = first_fv_idx_2d + (
        (nMax_face_nodes + 1) * np.arange(0, n_face))

    # column of first node values
    first_node_value = face_nodes[:, 0].copy()
...
def _build_nNodes_per_face(grid):
    ...
    value nodes for each face in ``face_nodes``"""

    # padding to shape [n_face, nMax_face_nodes + 1]
    closed = np.ones((grid.n_face, grid.nMax_face_nodes + 1),
                     dtype=INT_DTYPE) * INT_FILL_VALUE

    closed[:, :-1] = grid.face_nodes.copy()
...
def _build_edge_node_connectivity(grid, repopulate=False):
    """Constructs the UGRID connectivity variable (``edge_nodes``) and stores
    ...
    repopulate : bool, optional
        Flag used to indicate if we want to overwrite the existed `edge_nodes` and generate a new
        inverse_indices, default is False
    """

    # need to derive edge nodes
    if "edge_nodes" not in grid._ds or repopulate:
        padded_face_nodes = close_face_nodes(grid.face_nodes.values,
                                             grid.n_face, grid.nMax_face_nodes)

        # array of empty edge nodes where each entry is a pair of indices
        edge_nodes = np.empty((grid.n_face * grid.nMax_face_nodes, 2),
                              dtype=INT_DTYPE)
        ...
        # If "edge_nodes" already exists, directly return the function call
        return
    ...
    # add edge_nodes to internal dataset
    grid._ds['edge_nodes'] = xr.DataArray(
        edge_nodes_unique,
        dims=["n_edge", "Two"],
        attrs={
...
def _build_face_edges_connectivity(grid):
    """Constructs the UGRID connectivity variable (``face_edges``) and stores
    it within the internal (``Grid._ds``) and through the attribute
    (``Grid.face_edges``)."""
    if ("edge_nodes" not in grid._ds or
            "inverse_indices" not in grid._ds['edge_nodes'].attrs):
        _build_edge_node_connectivity(grid, repopulate=True)

    inverse_indices = grid._ds['edge_nodes'].inverse_indices
    inverse_indices = inverse_indices.reshape(grid.n_face, grid.nMax_face_nodes)

    grid._ds["face_edges"] = xr.DataArray(
        data=inverse_indices,
        dims=["n_face", "nMax_face_edges"],
        attrs={
