...
def get_GCA_GCA_intersection(gca1_cart, gca2_cart):
    ...
    if not np.allclose(np.dot(w0w1_norm, w0), 0, atol=ERROR_TOLERANCE) or not np.allclose(np.dot(w0w1_norm, w1), 0, atol=ERROR_TOLERANCE):
        raise ValueError("The current input data cannot be computed accurately using floating-point arithmetic. "
                         "Please turn on multi-precision mode and re-run.")

    if not np.allclose(np.dot(v0v1_norm, v0), 0, atol=ERROR_TOLERANCE) or not np.allclose(np.dot(v0v1_norm, v1), 0, atol=ERROR_TOLERANCE):
        raise ValueError("The current input data cannot be computed accurately using floating-point arithmetic. "
                         "Please turn on multi-precision mode and re-run.")

    if not np.allclose(np.dot(cross_norms, v0v1_norm), 0, atol=ERROR_TOLERANCE) or not np.allclose(np.dot(cross_norms, w0w1_norm), 0, atol=ERROR_TOLERANCE):
        raise ValueError("The current input data cannot be computed accurately using floating-point arithmetic. "
                         "Please turn on multi-precision mode and re-run.")
