...
#     """
#
#     # Special case if the latitude_cart is close to +1 or -1 (pole)
#     if isclose(latitude_cart, 1, atol=ERROR_TOLERANCE) or isclose(
#         latitude_cart, -1, atol=ERROR_TOLERANCE
#     ):
#         # Evenly distribute weights for all candidate faces
#         weights = {
#             face_index: 1 / len(faces_edges_cart_candidate)
#             for face_index in range(len(faces_edges_cart_candidate))
#         }
...
#             list(weights.items()), schema=["face_index", "weight"]
#         )
#         return weights_df
#
#     intervals_list = []
...
#     # Iterate through all faces and their edges
#     for face_index, face_edges in enumerate(faces_edges_cart_candidate):
#         # Remove any INT_FILL_VALUE data (ensure we only keep valid edges)
#         face_edges = face_edges[np.all(face_edges != INT_FILL_VALUE, axis=(1, 2))]
#
#         if is_face_GCA_list is not None:
#             is_GCA_list = is_face_GCA_list[face_index]
...
#             is_GCA_list = None
#
#         # This is presumably your own helper function that returns a Polars DataFrame
#         # with columns ["start", "end"] (and possibly others).
#         face_interval_df = _get_zonal_face_interval(
#             face_edges,
#             latitude_cart,
#             face_latlon_bound_candidate[face_index],
...
#         )
#
#         # 1) Check if there are any null values in the entire DataFrame
#         # Polars trick: select whether any of the columns has a null value
#         # The result is a single boolean in a DataFrame; we can extract it.
#         has_null = face_interval_df.select(pl.any(pl.all().is_null())).row(0)[0]
#         if has_null:
#             # Skip this face as it is being merely touched by the latitude
#             continue
#
#         # 2) Check if start==0 and end==0 for all rows
#         # This is the Polars equivalent of Pandas (df['start'] == 0).all()
#         all_start_zero = face_interval_df.select(
#             (pl.col("start") == 0).all()
#         ).row(0)[0]
...
#             continue
#
#         # 3) Iterate through the Polars DataFrame rows
#         #    - `iter_rows(named=True)` lets you get each row as a dictionary.
...
#             )
#
#     # Build a Polars DataFrame of intervals
#     intervals_df = pl.DataFrame(intervals_list)
#
#     try:
#         # This is presumably your own helper function that processes intervals
#         # and returns overlap_contributions (dict) and total_length (float).
