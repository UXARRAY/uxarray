...
def point_within_gca(pt, gca_cart, is_directed=False):
    ...
    if np.isclose(GCRv0_lonlat[0], GCRv1_lonlat[0], rtol=0.0, atol=ERROR_TOLERANCE):#GCRv0_lonlat[0] == GCRv1_lonlat[0]
        # If the pt and the GCA are on the same longitude (the y coordinates are the same)
        if np.isclose(GCRv0_lonlat[0], pt_lonlat[0], rtol=0.0, atol=ERROR_TOLERANCE):#GCRv0_lonlat[0] == pt_lonlat[0]
