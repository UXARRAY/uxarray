...
def __f1(x0, x1, y0, y1, z0, z1, x_i_old, y_i_old, z_lat):
    # X0 x X1 dot Xi=0
    return (y0 * z1 - z0 * y1) * x_i_old + (-x0 * z1 + z0 * x1) * y_i_old + (x0 * y1 - y0 * x1) * z_lat


def __f2(x_i_old, y_i_old, z_lat):
    # xi^2+yi^2+zi^2=1
...
def _newton_raphson_solver(init_cart, w0_cart, w1_cart, max_iter=1000):
    tolerance = 1.0e-17
    error = 9e9
    alpha = 1
    ...
    while error > tolerance and iter < max_iter:
        F[0] = __f1(w0_cart[0], w1_cart[0], w0_cart[1], w1_cart[1], w0_cart[2], w1_cart[2], y_guess[0], y_guess[1],
                    y_guess[2])
        F[1] = __f2(y_guess[0], y_guess[1], y_guess[2])
