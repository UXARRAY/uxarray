...
def _to_ugrid(in_ds, out_ds):
    ...
    if in_ds['grid_area'].all():
        ...
        # Create Grid2D_node_x/y variables from grid_corner_lat/lon
        ...
        # Now, calculate unique lon and lat values to account for 'node_x' and 'node_y'
        unq_lon = corner_lon_lat[unq_ind, :][:, 0]
        unq_lat = corner_lon_lat[unq_ind, :][:, 1]

        # Reshape face nodes array into original shape for use in 'face_nodes'
        unq_inv = np.reshape(unq_inv,
                             (len(in_ds.grid_size), len(in_ds.grid_corners)))

        # Create node_x/y from unsorted, unique grid_corner_lat/lon
        ...
        # Create face_x/y from grid_center_lat/lon
        out_ds['face_x'] = in_ds['grid_center_lon']
        out_ds['face_y'] = in_ds['grid_center_lat']
        ...
        out_ds["face_nodes"] = xr.DataArray(
            data=face_nodes,
...
def _encode_scrip(face_nodes, node_x, node_y, face_areas):
    ...
    f_nodes = face_nodes.values.astype(INT_DTYPE).ravel()

    # Create arrays to hold lat/lon data
    lat_nodes = node_y[f_nodes].values
    lon_nodes = node_x[f_nodes].values

    # Reshape arrays to be 2D instead of 1D
    reshp_lat = np.reshape(lat_nodes,
                           [face_nodes.shape[0], face_nodes.shape[1]])
    reshp_lon = np.reshape(lon_nodes,
                           [face_nodes.shape[0], face_nodes.shape[1]])
