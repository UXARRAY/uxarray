...
def _process_polygon(polygon: Polygon, node_lat_list: list, node_lon_list: list, face_conn_list: list, node_index: int) -> tuple:
    ...
    for lon, lat in polygon.exterior.coords[:-1]:  # Skip the last point as it's a duplicate of the first
    ...
    if polygon.interiors:
        warnings.warn("Interior rings (holes) in polygons are not currently supported and will be ignored.")
...
def _process_multipolygon(multipolygon: MultiPolygon, node_lat_list: list, node_lon_list: list, face_conn_list: list, node_index: int) -> tuple:
...
def _process_linestring(linestring: LineString, node_lat_list: list, node_lon_list: list, edge_conn_list: list, node_index: int) -> tuple:
    ...
    if len(coords) < 2:
        ...
        return node_lat_list, node_lon_list, edge_conn_list, node_index  # Not a valid edge sequence
...
def _process_multilinestring(multilinestring: MultiLineString, node_lat_list: list, node_lon_list: list, edge_conn_list: list, node_index: int) -> tuple:
...
def _process_point(point: Point, node_lat_list: list, node_lon_list: list, node_index: int) -> tuple:
...
def _process_multipoint(multipoint: MultiPoint, node_lat_list: list, node_lon_list: list, node_index: int) -> tuple:
...
def _read_geodataframe(filepath, driver=None, **kwargs):
    ...
    **kwargs
    ...
    node_lon, node_lat, face_node_connectivity, edge_node_connectivity = _extract_geometry_info(gdf)
    ...
    if face_node_connectivity.size > 0:
        grid_ds[ugrid.FACE_DIM] = np.arange(face_node_connectivity.shape[0], dtype=INT_DTYPE)
        grid_ds[ugrid.N_MAX_FACE_NODES_DIM] = np.arange(face_node_connectivity.shape[1], dtype=INT_DTYPE)
        ...
        grid_ds[ugrid.N_MAX_FACE_NODES_DIM] = np.arange(max_nodes_per_face, dtype=INT_DTYPE)
        source_dims_dict["n_face"] = ugrid.FACE_DIM
        source_dims_dict["n_max_face_nodes"] = ugrid.N_MAX_FACE_NODES_DIM
    ...
    if edge_node_connectivity.size > 0:
        grid_ds[ugrid.EDGE_DIM] = np.arange(edge_node_connectivity.shape[0], dtype=INT_DTYPE)
...
def _extract_geometry_info(gdf) -> tuple:
    ...
    for _, row in gdf.iterrows():
        ...
        if isinstance(geometry, Polygon):
            node_lat_list, node_lon_list, face_connectivity_list, node_index = _process_polygon(
                geometry, node_lat_list, node_lon_list, face_connectivity_list, node_index
            )
        elif isinstance(geometry, MultiPolygon):
            node_lat_list, node_lon_list, face_connectivity_list, node_index = _process_multipolygon(
                geometry, node_lat_list, node_lon_list, face_connectivity_list, node_index
            )
        elif isinstance(geometry, LineString):
            node_lat_list, node_lon_list, edge_connectivity_list, node_index = _process_linestring(
                geometry, node_lat_list, node_lon_list, edge_connectivity_list, node_index
            )
        elif isinstance(geometry, MultiLineString):
            node_lat_list, node_lon_list, edge_connectivity_list, node_index = _process_multilinestring(
                geometry, node_lat_list, node_lon_list, edge_connectivity_list, node_index
            )
            ...
            warnings.warn(f"Unsupported geometry type encountered: {geom_type}. Skipping.")
    ...
    if face_connectivity_list:
        ...
        face_node_connectivity = np.empty((0, max(1, max_nodes_per_face)), dtype=INT_DTYPE)
